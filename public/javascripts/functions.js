...
/*
 * DO NOT CALL FROM JAVASCRIPT/BROWSER.
 */
function checkFunctionsJS( aspPath, jsPath ) {
	Application.Lock();
	if ( aspPath == null )
		aspPath = "includes\\functions.asp";
	if ( jsPath == null )
		jsPath = "js\\functions.js";

	try {
		var objFSO = Server.CreateObject( "Scripting.FileSystemObject" );
		var rootDir = Server.MapPath( Request.ServerVariables( "SCRIPT_NAME" ) ).replace( /^(.*\\)([a-zA-Z0-9_\-]*\.asp)/, "$1" );
		var createJS = true;
		var funcASP = {
			"path": sprintf( "%s%s", rootDir, aspPath ),
			"file": null,
			"dt": null
		};
		var funcJS = {
			"path": sprintf( "%s%s", rootDir, jsPath ),
			"file": null,
			"dt": null
		};

		if ( objFSO.FileExists( funcASP["path"] ) )
			funcASP["file"] = objFSO.GetFile( funcASP["path"] );
		if ( objFSO.FileExists( funcJS["path"] ) )
			funcJS["file"] = objFSO.GetFile( funcJS["path"] );

		if ( funcASP["file"] != null ) {
			if ( funcJS["file"] != null ) {
				funcASP["dt"] = ( new Date( funcASP["file"].DateLastModified ) ).getTime();
				funcJS["dt"] = ( new Date( funcJS["file"].DateLastModified ) ).getTime();
				if ( funcASP["dt"] <= funcJS["dt"] )
					createJS = false;
			}
		} else {
			createJS = false;
		...
		if ( createJS ) {
			if ( funcJS["file"] != null )
				objFSO.DeleteFile( funcJS["path"], true );
			var objStream = Server.CreateObject( "ADODB.Stream" );
			objStream.Open();
			objStream.Type = 2;
			objStream.CharSet = "Windows-1252";
			objStream.LoadFromFile( funcASP["path"] );
			var openTag = "<" + "%";
			var closeTag = "%" + ">";
			var data = objStream.ReadText();
			objStream.Close();
			data = data.replace( openTag, "" ).replace( closeTag, "" );
			objStream.Open();
			objStream.WriteText( "//\n" );
			objStream.WriteText( "// *********** DO NOT MODIFY THIS FILE ***********\n" );
			objStream.WriteText( "// This file was automatically generated from functions.asp.\n" );
			objStream.WriteText( "// Generated: " + fmtDate( new Date(), "%0D %T" ) + "\n" );
			objStream.WriteText( "//\n" );
			objStream.WriteText( "function getGenDate() {\n" );
			objStream.WriteText( "\treturn fmtDate( new Date(), \"%0D %T\" );\n" );
			objStream.WriteText( "}\n" );
			objStream.WriteText( data );
			objStream.SaveToFile( funcJS["path"], 2 );
			objStream.Close();
		}
	} catch ( ex ) {
		reportError( "Unable to recreate functions.js", ex );
	}
	Application.Unlock();
}
